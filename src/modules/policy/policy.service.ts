...
export class PolicyService {
  ...
  async create(createPolicyDto: CreatePolicyDto) {
    try {
      const { clientId, agentId, planId, dependents, ...policyData } =
        createPolicyDto;
  ...
  async update(id: string, updatePolicyDto: UpdatePolicyDto) {
    const { clientId, agentId, planId, dependents, ...toUpdate } =
      updatePolicyDto;
  ...
  private handleDBErrors(error: any): never {
    if (error.code === '23503')
      throw new BadRequestException(
        'Foreign key constraint violation (Client, Agent or Plan ID invalid)',
      );
